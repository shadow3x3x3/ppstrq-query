...
async def fetch_detail(session, code, word):
    ...
    main_div = detail_soup.find("div", {"id": "formInput"})

    name_divs = main_div.find_all(
        "div", {"class": "well pubDetailWellHalf center-block"}
    )
    basic_response["債務人(買受人、受託人)名稱"] = (
        name_divs[0].find("div", {"class": "row"}).text[4:].strip()
    )

    basic_response["抵押權人(出賣人、信託人)名稱"] = (
        name_divs[1].find("div", {"class": "row"}).text[4:].strip()
    )

    main_divs = main_div.find_all("div", {"class": "well pubDetailWell center-block"})

    basic_info_div = main_divs[0]
    basic_response["案件類別"] = (
        basic_info_div.find("div", {"class": "row pubDetailRow"})
        .find_all("div", {"class": "col-sm-3 pubDetailValue"})[1]
        .text
    )

    trade_info_div = main_divs[-1]
    trade_info_divs = trade_info_div.find_all("div", {"class": "row pubDetailRow"})
    ...
    money = (
        trade_info_divs[1].find_all("div", {"class": "col-sm-3 pubDetailValue"})[1].text
    )
    basic_response["擔保債權金額"] = re.sub("\r|\n|\t", "", money)

    kind = trade_info_divs[-1].find("div", {"class": "col-sm-9 pubDetailValue"}).text
...
async def main(loop):
    async with aiohttp.ClientSession() as session:
        ...
        for tr in base_soup.tbody.find_all("tr"):
            ...
            tasks.append(
                loop.create_task(fetch_detail(session, detail_info[0], detail_info[1]))
            )
